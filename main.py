...
class RecipeFormPage:
    def __init__(self, parent_app, recipe_id=None):
        ...
        # ====== ΤΟΠΙΚΗ ΚΟΝΣΟΛΑ ΦΟΡΜΑΣ ======
        self.console_frame = ttk.LabelFrame(self.window, text="Μηνύματα Φόρμας")
        self.console_frame.grid(row=12, column=0, columnspan=2, padx=10, pady=5, sticky="ew")

        # Φτιάχνουμε το πεδίο κειμένου (πιο μικρό, height=4)
        self.local_console = tk.Text(self.console_frame, height=4, bg="#f4f4f4", state='disabled', font=("Consolas", 9))
        self.local_console.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5, pady=5)

        self.local_scroll = ttk.Scrollbar(self.console_frame, command=self.local_console.yview)
        self.local_scroll.pack(side=tk.RIGHT, fill=tk.Y)
        self.local_console.config(yscrollcommand=self.local_scroll.set)
    ...
    def save(self):
        ...
        name = self.name_entry.get().strip()
        if not name:
            self.log_local_message("Το όνομα της συνταγής είναι υποχρεωτικό.", "ERROR")
            return

        # Έλεγχος εγκυρότητας: επιλογή δυσκολίας
        difficulty = self.difficulty_var.get()
        if difficulty not in ["Εύκολη", "Μέτρια", "Δύσκολη"]:
            self.log_local_message("Παρακαλώ επιλέξτε έγκυρο βαθμό δυσκολίας.", "ERROR")
            return
        ...
        try:
            ...
            if total_time <= 0:
                self.log_local_message("Ο συνολικός χρόνος πρέπει να είναι θετικός αριθμός.", "ERROR")
                return
        except ValueError:
            self.log_local_message("Ο συνολικός χρόνος πρέπει να είναι έγκυρος ακέραιος.", "ERROR")
            return

        # Έλεγχος: τουλάχιστον ένα συστατικό και ένα βήμα
        if not self.ingredients:
            self.log_local_message("Παρακαλώ προσθέστε τουλάχιστον ένα συστατικό.", "ERROR")
            return
            
        if not self.steps:
            self.log_local_message("Παρακαλώ προσθέστε τουλάχιστον ένα βήμα.", "ERROR")
            return
        ...
        self.parent_app.log_message(f"Η συνταγή '{name}' αποθηκεύτηκε επιτυχώς!", "SUCCESS")
    ...
    def open_edit_ingredient_form(self):
        ...
        if not selected:
            self.log_local_message("Επιλέξτε ένα συστατικό για επεξεργασία.", "ERROR")
            return
    ...
    def delete_ingredient(self):
        ...
        selected = self.ingredients_tree.selection()  # Αλλαγή από curselection σε selection
        if not selected:
            self.log_local_message("Επιλέξτε ένα συστατικό για διαγραφή.", "ERROR")
            return
    ...
    def open_edit_step_form(self):
        ...
        if not selected:
            self.log_local_message("Επιλέξτε ένα βήμα για επεξεργασία.", "ERROR")
            return
    ...
    def delete_step(self):
        ...
        selected = self.steps_tree.selection()  # Αλλαγή από curselection σε selection
        if not selected:
            self.log_local_message("Επιλέξτε ένα βήμα για διαγραφή.", "ERROR")
            return
    ...
    def log_local_message(self, message, msg_type="INFO"):
            """Γράφει το μήνυμα στην τοπική κονσόλα ΚΑΙ το στέλνει στην κεντρική"""
            self.local_console.config(state='normal')
        
            # Εισαγωγή ώρας
            timestamp = datetime.datetime.now().strftime("%H:%M:%S")
        
            if msg_type == "ERROR":
                prefix = f"[{timestamp}] ❌ ΣΦΑΛΜΑ: "
            elif msg_type == "SUCCESS":
                prefix = f"[{timestamp}] ✅ ΕΠΙΤΥΧΙΑ: "
            else:
                prefix = f"[{timestamp}] ℹ️ ΠΛΗΡΟΦΟΡΙΑ: "
            
            full_message = prefix + message + "\n"
            self.local_console.insert(tk.END, full_message)
            self.local_console.see(tk.END)
            self.local_console.config(state='disabled')
...
class RecipeApp:
    def __init__(self, root):
        ...
        # Φτιάχνουμε ένα πλαίσιο (Frame) στο κάτω μέρος
        self.console_frame = ttk.LabelFrame(root, text="Κονσόλα Εφαρμογής")
        self.console_frame.grid(row=3, column=0, columnspan=4, padx=10, pady=5, sticky="ew")

        # Φτιάχνουμε το πεδίο κειμένου (μόνο για ανάγνωση -> state='disabled')
        self.console_text = tk.Text(self.console_frame, height=6, bg="#f4f4f4", state='disabled', font=("Consolas", 9))
        self.console_text.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5, pady=5)

        # Μπάρα κύλισης για την κονσόλα
        ...
        self.view_recipes()
        self.log_message("Η εφαρμογή ξεκίνησε με επιτυχία.", "INFO") # Αρχικό μήνυμα!
        # Αρχική φόρτωση των συνταγών
        self.view_recipes()

    def log_message(self, message, msg_type="INFO"):
        self.console_text.config(state='normal')  # Ξεκλειδώνουμε το κείμενο για να γράψουμε
        
        # Παίρνουμε την τρέχουσα ώρα
        timestamp = datetime.datetime.now().strftime("%H:%M:%S")
        
        # Φτιάχνουμε το πρόθεμα ανάλογα με το είδος του μηνύματος
        if msg_type == "ERROR":
            prefix = f"[{timestamp}] ❌ ΣΦΑΛΜΑ: "
        elif msg_type == "SUCCESS":
            prefix = f"[{timestamp}] ✅ ΕΠΙΤΥΧΙΑ: "
        ...
        # Εισαγωγή του μηνύματος και αλλαγή γραμμής (\n)
        full_message = prefix + message + "\n"
        self.console_text.insert(tk.END, full_message)
        
        # Αυτόματο scroll στο τελευταίο μήνυμα
        self.console_text.see(tk.END)
        self.console_text.config(state='disabled') # Κλειδώνουμε ξανά το κείμενο
    ...
    def open_update_page(self):
        ...
        if not selected:
            self.log_message("Επιλέξτε μια συνταγή για ενημέρωση.", "ERROR")
            return
    ...
    def delete_recipe(self):
        ...
        if selected:
            ...
            self.log_message(f"Η συνταγή '{recipe_name}' διαγράφηκε.", "SUCCESS")
        else:
            self.log_message("Επιλέξτε μια συνταγή για διαγραφή.", "ERROR")
